...
root = Tk()
root.title("Ten Fast Fingers")
root.geometry("850x500")

words_shown = ttk.Label(root,text="Kelimeler birsürü, burada gözükecek\n ve iki satıdan oluşacak burası en üstte")
words_shown.grid(row=0,sticky=(E,S))
...
main_frame.grid(sticky=(to_everywhere))
main_frame.columnconfigure(0, weight=1)
main_frame.rowconfigure(0, weight=1)


to_write = ttk.Label(main_frame, text="Burada Kelimeler Belirecek")
to_write.grid(column=0, row=0, sticky=(left_to_right))


writing = StringVar()
writing_area = ttk.Entry(main_frame, textvariable=writing)
writing_area.grid(column=2, row=2)
button = ttk.Button(main_frame, text="Button")
